...
CONTAINER_RUN_CMD := docker run --name ${CONTAINER} -it --rm --net mongonet -v $(PWD):/opt/project/code ${IMAGE}
...
mongo:
	docker run -it --rm --name mongo --net mongonet mongo:3

.PHONY: default rebuild dev dev_init dev_install dev_shell clean_images update_license mongo
